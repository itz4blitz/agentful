...
```
1. Read product spec (auto-detect PRODUCT.md or .claude/product/)
...
```markdown
Task("backend", "Implement JWT login per PRODUCT.md auth section")
Task("frontend", "Create login page per PRODUCT.md auth UI spec")
...
[Read PRODUCT.md] → Auth spec found
[Read state.json] → No current work
[Read completion.json] → Auth = 0%

[Classify] → FEATURE_DEVELOPMENT (autonomous loop)

[Delegate] Task("backend", "Implement JWT auth API per PRODUCT.md")
[Wait] Backend completes

[Delegate] Task("frontend", "Create login page per PRODUCT.md")
...
The Orchestrator reads both formats and tracks completion accordingly.
