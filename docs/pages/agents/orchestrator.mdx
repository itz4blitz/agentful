...
If `.claude/product/product-analysis.json` exists (created by `/agentful-product`), the Orchestrator will:
...
6. Check .claude/product/product-analysis.json (readiness gates)
```
...
[Orchestrator] → Starts development work
  ↓
[Check] → .claude/product/product-analysis.json exists
  ↓
...
# Product readiness
cat .claude/product/product-analysis.json
```
