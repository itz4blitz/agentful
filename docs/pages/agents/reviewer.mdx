...
The Reviewer Agent **DETECTS** issues (it does not fix them - that's the Fixer's job). It runs 9 comprehensive checks to validate:
...
**Key Distinction:** Reviewer DETECTS, Fixer FIXES. When issues are found, the Reviewer saves them to `.agentful/last-review.json` and the Orchestrator delegates to the Fixer to resolve them.
...
The Reviewer runs **9 comprehensive checks**. All must pass for code to be production-ready.
...
Ensure critical code has proper documentation.

```bash
# Check for missing JSDoc on exported functions
grep -r "^export function" src/ --include="*.ts" --include="*.tsx" | \
  while read line; do
    file=$(echo "$line" | cut -d: -f1)
    func=$(echo "$line" | grep -oP "export function \K\w+")
    # Check if function has JSDoc comment above it
    grep -B1 "export function $func" "$file" | grep -q "/\*\*" || \
      echo "Missing documentation: $func in $file"
  done
...
test -d docs/ || echo "Consider adding docs/ directory"
```

**FAIL if:** Missing critical documentation
...
```json
{
  ...
  "passed": false,
  "issues": [
    "Missing JSDoc: authenticateUser in src/services/auth.service.ts",
    "README.md exists but lacks setup instructions",
    "No API documentation found"
  ],
  "summary": "Found 3 documentation issues"
}
```

**Documentation requirements:**
- Public API functions should have JSDoc comments
- README.md should exist with setup instructions
...
```json
{
  ...
  "checks": {
    ...
    "documentation": {
      "passed": true,
      "summary": "All critical functions documented"
    },
    "manualReview": {
      "passed": true,
...
5. **NEVER** fix issues yourself (delegate to @fixer) - **REVIEWER DETECTS, FIXER FIXES**
...
9. **NEVER** modify code - you only validate and report
...
- [Fixer Agent](/agents/fixer) - Fixes issues detected by Reviewer
- [Tester Agent](/agents/tester) - Writes tests that Reviewer validates
- [Orchestrator Agent](/agents/orchestrator) - Coordinates the review workflow
- [/agentful-status](/commands/agentful-status) - Check project validation status
