...
Start or resume autonomous work loop by delegating to the orchestrator agent.

## Purpose

The `/agentful-start` command is the primary entry point for autonomous agent work. It:
...
- Continues the work loop until interrupted or complete
...
For continuous work, wrap in Ralph loop:
...
```
Task("orchestrator", "Run autonomous work loop. Read state, pick next task,
...
3. Orchestrator reads product spec (.claude/product/index.md)
    ...
    steps:
      ...
      - name: Commit progress
        run: |
          ...
          git add .
          git commit -m "feat: autonomous work progress"
          git push
```
