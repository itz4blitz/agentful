...
Complete reference guide for all agentful slash commands used in structured product development.

## Overview

agentful provides a set of slash commands that automate the product development lifecycle. These commands work together to create a structured development loop that can build features, validate quality, and resolve decisions with human checkpoints for key choices.

## Command Categories

### Planning Commands

- **[/agentful-product](./agentful-product)** - Smart product planning: create, analyze, and refine requirements

### Core Development Commands

- **[/agentful-start](./agentful-start)** - Start or resume structured product development
...
```
0. /agentful-product   → Define and refine requirements (optional)
1. /agentful-start     → Begin structured development
...
1. **Product specification** - Either `.claude/product/index.md` (preferred, hierarchical) or `PRODUCT.md` (also supported, flat structure)
...
### Workflow 3: Extended Development Sessions (Advanced)

For longer development sessions with fewer interruptions:

```bash
/ralph-loop "/agentful-start" --max-iterations 50 --completion-promise "AGENTFUL_COMPLETE"
```

This runs the development loop with pauses for:
- Human decisions that are needed
...
1. **Use extended sessions for long work periods** - Let the loop run with fewer interruptions while you focus on other tasks
2. **Check status between iterations** - Stay informed without interrupting flow
3. **Keep your product spec updated** - Use `.claude/product/index.md` (preferred) or `PRODUCT.md` (also supported) - Better requirements = better AI-assisted development
