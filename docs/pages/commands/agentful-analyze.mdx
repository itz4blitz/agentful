...
Gap Detection
  [CRITICAL] No product specification found
    Create PRODUCT.md or .claude/product/index.md
...
Staleness Detection
  [WARNING] frontend agent has low confidence (0.4)
    Run '/agentful-agents' to regenerate from actual code
...
```json
{
  "hooks": {
    "PostToolUse": [
      {
        "matcher": "Edit|Write",
        "command": "bin/hooks/analyze-trigger.sh \"$TOOL_INPUT\""
      }
    ],
    "SessionStart": [
      {
        "command": "bin/hooks/health-check.sh"
      }
    ]
  }
}
...
- **After `/agentful-agents`**: Run `quick` to verify agents generated correctly
- **After `/agentful-skills`**: Run `quick` to check skill alignment
...
- [/agentful-agents](/commands/agentful-agents) - Generate domain agents
- [/agentful-skills](/commands/agentful-skills) - Discover and generate skills
