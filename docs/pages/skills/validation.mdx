...
| **Coverage** | Minimum coverage | ≥80% required |
...
- Flow → Runs `flow check`
- JavaScript → Skips type checking
...
- Jest + Istanbul
- c8
- None → Warns but doesn't fail

**Pass Criteria:** ≥80% code coverage
...
The reviewer agent automatically runs validation:
...
The fixer agent uses validation to:
1. Identify issues that need fixing
2. Verify fixes are successful
...
**How it adapts:**
- No test framework → Skips (warns user)
- Monorepo → Tests each package
- Workspaces → Runs tests in dependency order
...
**How it adapts:**
- No tsconfig.json → Skips type checking
- JavaScript files → Doesn't fail on .js files
- @ts-check → Respects TypeScript comments
...
**What it checks:**
- ESLint configuration found → Runs `eslint .`
- Biome configuration found → Runs `biome check .`
- No linter → Skips
...
```
⚠️  Dead code found:
   • src/lib/unusedHelper.ts (exported but never imported)
   • src/components/OldButton.tsx (replaced by NewButton.tsx)
   • Import 'lodash' in utils.ts (unused)
```
...
**How it adapts:**
- No coverage tool configured → Warns but doesn't fail
- Custom threshold in package.json → Uses that instead
...
│ File            │ % Stmts │ % Branch │
├─────────────────────────────────┤
│ LoginForm.tsx  │   92.3  │   88.9   │
│ utils.ts       │   65.2  │   50.0   │
│ TaskItem.tsx    │   71.4  │   66.7   │
└─────────────────────────────────┘
```
...
- ⚠️  Warning: Low/medium severity
- ❌ Fail: High/critical severity
...
The validation skill (via fixer agent) can automatically fix:
