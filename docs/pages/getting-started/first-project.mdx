...
agentful analyzes your Next.js project and:
1. Detects tech stack (Next.js, TypeScript, Tailwind)
2. Finds 2 domains (Task Management, UI Components)
3. Creates `PRODUCT.md` (< 3 domains = flat structure)

If you had ≥3 domains, it would create `.claude/product/` with domain directories.
...
## Step 3: Start Claude and Generate Agents
...
This generates specialized agents for your stack:
...
Update acceptance criteria in `PRODUCT.md`, then run `/agentful-generate`.

### Switch to Hierarchical Structure
...
2. **Product Structures** - Flat (PRODUCT.md) or hierarchical (.claude/product/), both auto-detected
