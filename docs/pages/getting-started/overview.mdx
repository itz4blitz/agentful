...
### 3. Context-Driven Development (Spec Optional)

Most teams use agentful conversationally first (`/agentful "build X"`), then add more structure only when needed.

- **Conversation-first** - Start work with `/agentful` and natural language requests
- **Stateful progress** - `.agentful/` tracks current phase, completion, and decisions
- **Optional specs** - `.claude/product/` improves planning for larger projects but is not required to begin
- **Escalation path** - Move from chat requests to structured specs when scope grows

The orchestrator uses available context (conversation history, state files, and optional specs) to choose the next best task.
...
```mermaid
graph TD
    User["👤 Developer"] -->|/agentful or /agentful-start| Orchestrator["🎯 Orchestrator<br/>Coordinates & Delegates"]
...
**How it works:**
1. You invoke `/agentful` (or `/agentful-start`) with a task
...
The typical workflow is progressive:

### 1. Set Direction (Conversation First)

You can start immediately with natural language:

```bash
/agentful "build auth and roles"
/agentful "fix flaky tests in billing"
/agentful "keep going"
```

For larger projects, optionally add structure in `.claude/product/`:
...
Run `/agentful-start` (or `/agentful "keep going"`) to begin or continue work:

**On first run**, `/agentful-start` automatically:
- Analyzes your codebase and detects tech stack
- Generates specialized agents based on your code patterns
- Creates tech skills from your conventions
- Then begins development

**For new projects** (no code yet):
- You can start with `/agentful` and describe what to build
- If needed, run `/agentful-init` or `/agentful-product` to add more structure

**For existing projects**:
- Scans code to discover business domains
- Generates domain-specific agents (e.g., `@auth`, `@billing`)
- Creates tech skills (e.g., React patterns, Prisma patterns)

**Then it**:
- Orchestrator reads available context (state, history, optional product docs)
...
```mermaid
graph LR
    A["🧭 User Request / Project Context"] -->|Analyze| B["🔍 Requirements Check"]
    B -->|Complete| C["💻 Implementation"]
    B -->|Gaps found| D["❓ Ask Human"]
    D --> C

    C -->|Backend| E1["⚙️ API Development"]
    C -->|Frontend| E2["🖥️ UI Development"]
...
1. **[Installation Guide](/getting-started/installation)** - Choose default, minimal, or custom CLI setup
...
- **Community**: [Discord](https://discord.gg/X2tCJh76rG)
