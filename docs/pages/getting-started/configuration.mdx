...
agentful supports flat and hierarchical structures with automatic detection.

### Flat Structure

Best for quick prototypes, MVPs, small projects.
...
agentful auto-detects which format you're using:
1. Hierarchical: If `.claude/product/domains/*/index.md` exists
2. Flat (.claude): If `.claude/product/index.md` exists (no domains)
3. Flat (legacy): If `PRODUCT.md` exists at root
...
|--------|----------|----------|
| Flat (PRODUCT.md) | Very small project | Simplest setup |
| Flat (.claude/product/index.md) | Want organization, small project | Keeps root clean |
| Hierarchical (.claude/product/domains/) | Most projects, 20+ features | Domain-driven, scalable (preferred) |
