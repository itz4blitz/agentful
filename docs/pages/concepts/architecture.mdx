...
graph TD
    ...
    Orchestrator --> PA[📊 Product Analyzer<br/>Requirements & Gaps]
    Orchestrator --> Architect[🏗️ Architect<br/>Tech Stack & Design]
    Orchestrator --> Backend[⚙️ Backend<br/>APIs & Database]
    Orchestrator --> Frontend[🎨 Frontend<br/>UI & State]
    Orchestrator --> Tester[🧪 Tester<br/>Tests & Coverage]
    Orchestrator --> Reviewer[🔍 Reviewer<br/>Quality Gates]
    Orchestrator --> Fixer[🔧 Fixer<br/>Auto-remediation]
    Orchestrator -.-> Custom[➕ Custom Agents<br/>Domain-specific]
    ...
    style PA fill:#3b82f6,stroke:#2563eb,color:#fff
    style Architect fill:#8b5cf6,stroke:#7c3aed,color:#fff
    style Backend fill:#06b6d4,stroke:#0891b2,color:#fff
    style Frontend fill:#f59e0b,stroke:#d97706,color:#fff
    style Tester fill:#ec4899,stroke:#db2777,color:#fff
    style Reviewer fill:#14b8a6,stroke:#0d9488,color:#fff
    style Fixer fill:#ef4444,stroke:#dc2626,color:#fff
    style Custom fill:#64748b,stroke:#475569,color:#fff
```
...
sequenceDiagram
    ...
    Fix->>O: Errors fixed
    O->>R: Re-validate
    R->>O: All gates passed ✓

    Note over O,PA: Planning Phase
    Note over B,T: Implementation Phase
    Note over R,Fix: Validation Phase
```
...
1. **Detects the issue** - Identifies missing files, invalid JSON, or missing required fields
2. **Backs up corrupted files** - Creates timestamped backup (e.g., `state.json.backup-1706198400`)
3. **Creates fresh file with defaults** - Initializes new file with schema-defined default values
4. **Logs recovery action** - Reports what was recovered and how
...
- [State Files Reference](/reference/state-schema) - Complete schema definitions
- [State Validator API](/reference/state-validator) - Validation and recovery API
