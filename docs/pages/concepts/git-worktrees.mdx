...
├── .agentful/                   # Main worktree state
├── .claude/product/index.md     # Git-tracked spec
└── src/

# Additional worktree structure
repo-dashboard/
├── .git -> repo/.git/worktrees/repo-dashboard/  # Links to shared git
├── .agentful/                   # Independent state
├── .claude/product/index.md     # Different branch = different spec
└── src/
```
...
Consider using [remote execution](/remote-execution) instead.
...
For long-running parallel development that requires coordination, consider [`agentful serve`](/remote-execution):
