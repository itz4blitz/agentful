...
Orchestrator
  ...
  Backend executes → Returns
  ↓
  Task("frontend", "Create UI")
  ↓ [BLOCKING - waits for completion]
  Frontend executes → Returns
  ↓
...
```
0:00 → backend starts
0:05 → backend completes, frontend starts
0:10 → frontend completes, tester starts
0:15 → tester completes, reviewer starts
0:18 → reviewer completes, ALL DONE
```
...
Sequential (current):
```
Feature A: backend (5min) → frontend (5min) → tester (5min)
Feature B: backend (5min) → frontend (5min) → tester (5min)
Feature C: backend (5min) → frontend (5min) → tester (5min)
Feature D: backend (5min) → frontend (5min) → tester (5min)
...
```
Feature A: [backend → frontend → tester] (15min)
Feature B: [backend → frontend → tester] (15min)  } All running
Feature C: [backend → frontend → tester] (15min)  } in parallel
Feature D: [backend → frontend → tester] (15min)

Total: 15 minutes (4x faster!)
```
