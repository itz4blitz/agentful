...
**Note:** Ralph Wiggum is an external plugin that requires separate installation before use.

```bash
# Inside Claude Code
# First, install the Ralph Wiggum plugin:
/plugin install ralph-wiggum@anthropics

# Then run autonomous development loop:
...
├── .agentful/              # Runtime state (excluded from version control)
